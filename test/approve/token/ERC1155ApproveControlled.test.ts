...
describe("ERC1155ApproveControlled", function () {
  async function fixture() {
    const [owner, alice, bob, carol, user1, user2, nonUser1, nonUser2] = await ethers.getSigners();

    const [MockERC1155ApproveControlled, ApproveController] = await Promise.all([
      ethers.getContractFactory("MockERC1155ApproveControlled", owner),
    ...
    const [token1, token2] = await Promise.all([
      MockERC1155ApproveControlled.deploy(controller.address),
      MockERC1155ApproveControlled.deploy(controller.address),
    ]);
    ...
    await Promise.all([
      ...
      ...[alice, bob, user1, user2, nonUser1, nonUser2].flatMap((signer) => [
        ...[token1, token2].flatMap(async (token) => token.mint(await signer.getAddress(), 1, 10000, "0x")),
      ]),
  ...
  describe("safeTransferFrom", function () {
    it("should not be reverted if approved by controller", async function () {
      ...
      await expect(token1.connect(alice).safeTransferFrom(await user1.getAddress(), recipientAddress, 1, 1n, "0x")).not
        .to.be.reverted;
    });
    ...
    it("should be reverted when operator tries to spend unapproved token", async function () {
      ...
      await expect(
        token2.connect(alice).safeTransferFrom(await nonUser1.getAddress(), recipientAddress, 1, 1n, "0x")
    ...
    it("should be reverted when non-operator tries to spend token approved by controller", async function () {
      ...
      await expect(
        token2.connect(user1).safeTransferFrom(await user2.getAddress(), recipientAddress, 1, 1n, "0x")
      ).to.be.revertedWith(nxErrors.ERC1155.transferForbidden);
    });
    ...
    it("should not be reverted when spending manually approved owner's token", async function () {
      ...
      await expect(
        token2.connect(nonUser1).safeTransferFrom(await nonUser2.getAddress(), recipientAddress, 1, 1n, "0x")
      ).not.to.be.reverted;
    });
    ...
    it("should not be reverted when operator spends manually approved owner's token", async function () {
      ...
      await expect(token2.connect(bob).safeTransferFrom(await nonUser2.getAddress(), recipientAddress, 1, 1n, "0x")).not
        .to.be.reverted;
    });
    ...
    it("should not be reverted when operator spends its own token", async function () {
      ...
      await expect(token1.connect(bob).safeTransferFrom(await bob.getAddress(), recipientAddress, 1, 1n, "0x")).not.to
        .be.reverted;
    });
    ...
    it("should not be reverted when approver spends its own token", async function () {
      ...
      await expect(token1.connect(user1).safeTransferFrom(await user1.getAddress(), recipientAddress, 1, 1n, "0x")).not
        .to.be.reverted;
    });
    ...
    it("should not be reverted when non-approver spends its own token", async function () {
      ...
      await expect(
        token1.connect(nonUser1).safeTransferFrom(await nonUser1.getAddress(), recipientAddress, 1, 1n, "0x")
      ).not.to.be.reverted;
    });
    ...
    it("executor is not operator", async function () {
      ...
      await expect(
        token2.connect(carol).safeTransferFrom(await user1.getAddress(), recipientAddress, 1, 1n, "0x")
      ).to.be.revertedWith(nxErrors.ERC1155.transferForbidden);
    });
    ...
    it("owner is not operator", async function () {
      ...
      await expect(
        token2.connect(owner).safeTransferFrom(await user1.getAddress(), recipientAddress, 1, 1n, "0x")
      ).to.be.revertedWith(nxErrors.ERC1155.transferForbidden);
    });
  });
